...
prolog:message(retract_failed(Ax)) -->
        [ 'retract_failed ~w'-[Ax] ].

retract_chk(OAxiom) :-
	debug(owl2_model, 'retract_chk: ~w', [OAxiom]),
	( retract(OAxiom) -> true ; print_message(warning, retract_failed(OAxiom)) ).

retract_all_axioms :-
        findall(A,axiom(A),Axioms),
        maplist(retract_chk,Axioms),
        findall(ontologyAxiom(O,A),ontologyAxiom(O,A),OAxioms),
        maplist(retract_chk,OAxioms),
	!.
